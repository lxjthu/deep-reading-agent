...
### 2.5 模块五：社科文献深度阅读 (Social Science Scholar)
*   **入口**: `run_social_science_task.py` -> `social_science_analyzer.py`
*   **目标**: 针对管理学/社会学定性研究文献，采用**“四层金字塔”模型**进行深度情报提取。
*   **架构**:
    *   **L1 Context (背景)**: 提取政策背景、现状数据。
    *   **L2 Theory (理论)**: 提取核心构念、理论框架、过往理论总结。
    *   **L3 Logic (逻辑)**: 提取案例过程模型、因果组态路径。
    *   **L4 Value (价值)**: 提取研究缺口、理论贡献、实践启示。
*   **产出**:
    *   **4+1+1 结构**: 4个分层MD + 1个全景报告MD + 1个Excel汇总表。
    *   **双向链接**: 自动注入 Obsidian 兼容的 `[[Link]]`，实现文档间快速跳转。
    *   **中文强制**: 针对中文文献优化 Prompt，确保输出纯正中文。
...
### 已完成功能 (Done)
- [x] **社科文献深度阅读 Skill**: 完成了基于“四层金字塔”模型的深度阅读模块开发。
    - [x] 实现 `social_science_analyzer.py` 核心逻辑。
    - [x] 实现 `run_social_science_task.py` 自动化工作流（切分->分析->链接）。
    - [x] 实现 `link_social_science_docs.py` 双向链接注入。
