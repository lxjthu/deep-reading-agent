...
async def upload_entry_markdown(
    ...
    if not original_name:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="缂哄皯鏂囦欢鍚嶃€?)
    file_ext = Path(original_name).suffix.lower()
    if file_ext not in {".md", ".markdown"}:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="璇蜂笂浼?Markdown 鏂囦欢銆?)
...
async def upload_attachment(
    ...
    file_ext = Path(original_name).suffix.lower()
    if file_ext not in {".md", ".markdown"}:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="闄勪欢浠呮敮鎸?Markdown 鏂囦欢銆?)
    ...
    try:
        ...
        if existing_link is not None:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="璇ユ枃浠跺凡浣滀负闄勪欢鎸傝浇銆?)
...
async def update_attachment(
    ...
    if att is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="闄勪欢涓嶅瓨鍦ㄣ€?)
...
async def search_entry_fulltext(
    ...
    for candidate in lookup.pdf_candidates[:5]:
        try:
            ...
            return FullTextLookupResponse(
                status="attached",
                message=f"宸叉壘鍒板苟鎸傝浇 {candidate.label}銆?,
    ...
    message = (
        "鏈兘鑷姩鎸傝浇 PDF锛屽凡杩斿洖 DOI/寮€鏀捐闂〉闈㈠拰 working paper 妫€绱㈠叆鍙ｃ€?
        if status_text == "landing_only"
        else "鏈兘鑷姩鎸傝浇 PDF锛屽凡杩斿洖 working paper 妫€绱㈠叆鍙ｃ€?
    )
    return FullTextLookupResponse(
        status=status_text,
        message=message,
...
async def get_entry_reader(
    ...
    versions = [
        ReaderVersion(version="original", label="鍘熸枃", available=markdown_file is not None),
        ReaderVersion(version="translated", label="璇戞枃", available=translation_row is not None),
    ]
    ...
    if is_attachment_view:
        ...
        if att_file_id not in attachment_file_map:
            if markdown_file is not None:
                ...
                raise HTTPException(status_code=404, detail="璇锋眰鐨勯檮浠朵笉瀛樺湪銆?)
    else:
        ...
        if current == "original" and markdown_file is None and not attachment_rows:
            raise HTTPException(status_code=404, detail="褰撳墠鏂囩尞灏氭湭缁戝畾 Markdown 鍘熸枃銆?)
    ...
    if is_attachment_view:
        ...
        if markdown_file is None:
            raise HTTPException(status_code=404, detail="褰撳墠鏂囩尞鍘熸枃鏂囦欢宸蹭涪澶憋紝璇烽噸鏂颁笂浼?Markdown 鍘熸枃銆?)
...
async def match_online(
    ...
    if entry is None:
        raise HTTPException(status_code=404, detail="鏂囩尞涓嶅瓨鍦ㄣ€?)
...
async def apply_match(
    ...
    if candidate_source == "local":
        ...
        if other_entry is None:
            raise HTTPException(status_code=400, detail="鏈湴鍊欓€夋枃鐚笉瀛樺湪銆?)
...
async def wos_search(
    ...
    if not entry.title or not entry.title.strip():
        raise HTTPException(status_code=400, detail="鏂囩尞鏍囬涓虹┖锛屾棤娉曟悳绱€?)
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"WOS 鎼滅储澶辫触锛歿exc}") from exc
