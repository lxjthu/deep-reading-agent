...
- **AI 文献助手**（2026-05-22）
  ...
  - **持久会话与执行确认**（2026-05-23）：`agent_sessions` / `agent_messages` / `agent_action_proposals` 三表持久化，执行型工具走 proposal-first 确认流程
  - **UI 增强**（2026-05-23）：输入框下移至回答下方、AI 回答 Markdown 渲染（含 KaTeX 数学公式）、工具调用中间态折叠、会话归档清空上下文（数据保留供记忆系统）、导出对话为 Markdown（全选/部分选择）
