...
**功能**：将经济学论文（PDF 或 MD）用规范的中文学术语言重述，专为计量经济学文献设计。PDF 文件自动完成提取后再重述；MD 文件直接进入重述流程。

**5 步重述流水线**：

| 步骤 | 内容 | 说明 |
|------|------|------|
| Step 1 | 提取标题、摘要、引言 | 自动识别 YAML frontmatter、`## Abstract`、`## Introduction` 等 |
| Step 2 | 生成术语词典 | 调用 DeepSeek 提取经济学专业术语，输出四类分组对照表 |
| Step 3 | 检测章节标题层级 | 读取前 ~3 页，由 DeepSeek 判断 `##` 还是 `###` 作为分节标题 |
| Step 4 | 按标题切块 | 按检测到的层级切块，相邻小节合并，超大节按段落二次切分 |
| Step 5 | 并行逐块重述 | 多线程并发调用 DeepSeek，保序拼接，自动去除说明性开场白 |

**两套专用提示词**：

| 提示词 | 触发时机 | 作用 |
|--------|----------|------|
| 术语词典提示词 | Step 2，调用一次 | 基于标题+摘要+引言，生成英中对照表，含计量方法、变量、政策名称等四类分组 |
| 逐块重述提示词 | Step 5，每块调用一次 | 注入词典确保全文术语统一，要求内容完整、格式保留、符合中文经济学期刊规范 |
...
|------|------|--------|
| 模式 | 单文件 或 批量文件夹 | 单文件 |
| 上传文件 | 单文件模式：上传 PDF 或 MD 文件 | - |
| 文件夹路径 | 批量模式：支持 PDF/MD 混合文件夹 | - |
| PDF 提取方式 | PaddleOCR 本地GPU / 远程API / Legacy (pdfplumber)，仅 PDF 文件生效 | 本地GPU（如可用） |
| 重述模型 | `deepseek-chat`（快、省）或 `deepseek-reasoner`（高质量） | deepseek-chat |
| 每块最大字符数 | 控制分块粒度（2000–8000 字符） | 5000 |
| 并发重述数 | 同时发出的 API 请求数（1–10） | 5 |
| **开始重述** | 启动流水线 | - |
| **停止** | 取消（在当前 API 调用完成后生效） | - |

**右栏（结果）**：

| 控件 | 说明 |
|------|------|
| 运行日志 | 实时流式日志（步骤进度 + 每块完成情况） |
| 处理进度 | 进度表格（文件名 / 类型 / 状态 / 耗时），批量模式下更新 |
| 总体进度 | 完成/失败统计 |
| 中文重述预览 | 单文件模式完成后预览重述内容（前 15000 字符） |
| 下载中文版 MD | 单文件模式的 `{stem}_cn.md` 下载 |

**输出文件**：

```
translation_results/{论文名}/
├── {论文名}_cn.md         # 中文重述正文（含 lang: zh-cn 等 YAML 字段）
└── {论文名}_glossary.md   # 术语词典（经济学专业术语对照表）
```

**并发拼接原理**：多线程完成顺序不确定，但采用预分配有序索引槽（`restated_parts[idx]`），每个线程只写自己的槽位，`ThreadPoolExecutor` 退出时所有槽已填满，按索引顺序拼接，保证输出与分块原始顺序严格一致。
